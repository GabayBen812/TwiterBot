...
# This project is now <a href='https://www.lazytrade.io'>www.lazytrade.io</a>. Execute crypto trades with one touch on Telegram!
...
Fixed issue with querying. New coin listings from coinbase, coinbasepro and kucoin successfully traded +25%. Works when BNB is in kucoin account to take fees. (March 15th)
...
The buy amount is input in $ and based on the latest prices from the exchange, the program  will calculate valid buy and (multiple) sell amounts as close to the specified $ amount as the market will allow (kucoin). BNB coin required in account to take fees. With Kraken, the buy amount has to be a valid tradeable amount in crypto and when divided by the number of selling trades, also has to leave valid tradeable amounts 

To configure on local system: \
`pip install -r requirements.txt`

To run with kucoin (monitors a single ticker): \
`python twitter_kucoin.py`
...
- Requires a crypto exchange (Kraken/kucoin) API which is used through ccxt (cryptocurrency exchange trading library)
